...
# Suppress JAX backend initialization warnings and configure for threading
os.environ["JAX_PLATFORMS"] = "cpu"
os.environ["TF_CPP_MIN_LOG_LEVEL"] = "3"
os.environ["JAX_ENABLE_X64"] = "False"
os.environ["JAX_TRACEBACK_FILTERING"] = "off"

# Disable JAX JIT compilation to avoid threading issues
import jax
...
def generate_fn(
    ...
    return NodeState(generation_result=result, eval_results=eval_results, model_name=model_name), score
...
def _process_task(
    ...
    # Parallel processing of nodes - use threading backend for macOS compatibility
    results = Parallel(n_jobs=n_jobs, backend="threading")(delayed(process_node)(node, task) for node in tqdm(valid_nodes, desc=f"Processing {task_id}"))
