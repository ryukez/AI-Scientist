...
class GeminiAPIModel(Model):
    ...
    def call_api(
        ...
        for content in messages:
            ...
            if "assistant" in content:  # Gemini API uses "model" instead of "assistant" as a role
                content = genai.types.ModelContent(parts=[genai.types.Part.from_text(text=content["content"])])
            if "content" in content:
                content = genai.types.UserContent(parts=[genai.types.Part.from_text(text=content["content"])])
            contents.append(content)
        return try_generate(self, contents, generation_config)

    def generate(self, messages: list[dict[str, str]], temperature: float = 0.7) -> tuple[str, float]:
        ...
        if self.model_name == "gemini-2.5-pro-preview-05-06":
            if prompt_token_count <= 200_000:
                prompt_price += prompt_token_count * PRICING[self.model_name]["prompt_token_count"]
            else:
                prompt_price += 200_000 * PRICING[self.model_name]["prompt_token_count"]
                prompt_price += (prompt_token_count - 200_000) * PRICING[self.model_name]["prompt_token_count"] * 2
            if candidates_token_count <= 200_000:
                candidates_price += candidates_token_count * PRICING[self.model_name]["candidates_token_count"]
            else:
                candidates_price += 200_000 * PRICING[self.model_name]["candidates_token_count"]
                candidates_price += (candidates_token_count - 200_000) * PRICING[self.model_name]["candidates_token_count"] * 1.5
